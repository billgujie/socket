...
int phase1(void) {
	...
	char *message = "File_Server2 42374";
	...
	printf(
			"Phase 1: The Registration request from File Server 2 has been sent to the Directory Server.\n");

	//loop, receiving data
	for (;;) {
		//waiting on port #23374
...
int phase3(void) {
	...
	printf("Phase 3: File Server 2 has TCP port %d and IP address %s.\n",
			PHASE3, readableIP);
	...
	while (1) {
		...
		if (recvlen > 0) {
			...
			client = identify(buf);
			printf(
					"Phase 3: File Server 2 received the request from the Client %d for the file doc%d.\n",
					client, client);

			//construct the doc string and send back to Client
			sprintf(message, "doc%d", client);
			send(incoming, message, strlen(message) + 1, 0);
			printf("Phase 3: File Server 2 has sent doc%d to Client %d.\n",
					client, client);
			//close(incoming);
		}
	}
