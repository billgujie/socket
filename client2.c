...
int phase2(int remote_info[2]) {
	...
	printf(
			"Phase 2: The File request from Client 2 has been sent to the Directory Server.\n");

	//loop, receiving data
	int file_server;
	int server_tcp;
	for (;;) {
		//waiting on port #33374
		...
		if (recvlen > 0) {
			...
			printf(
					"Phase 2: The File requested by Client 2 is present in File Server %d and the File Server's TCP port number is %d.\n",
...
int phase3(int remote_info[2]) {
	...
	printf(
			"Phase 3: Client 2 has dynamic TCP port number %d and IP address %s.\n",
			self_addr.sin_port, readableIP);
	//strcpy(message, "doc2 ");
	send(tcp_sock, message, strlen(message)+1, 0);
	printf(
			"Phase 3: The File request from Client 2 has been sent to the File Server %d.\n",
			remote_info[0]);
	for (;;) {
		...
		if (recvlen > 0) {
			buf[recvlen] = 0;
			//printf("%s\n", buf);
			if (strncmp(buf, "doc2", 4) == 0) {
				//shutdown(tcp_sock, 2);
				close(tcp_sock);
				printf(
						"Phase 3: Client 2 received doc2 from File Server %d.\n",
						remote_info[0]);
